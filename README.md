...
## Оффлайн-установка зависимостей и сборка
Если сборка должна выполняться без доступа в интернет:

1. На машине с интернетом выполните:
   - `scripts\prepare_offline_wheels.bat`
2. Перенесите папку `vendor\wheels` вместе с проектом на оффлайн-машину.
3. Запустите обычную сборку:
   - `scripts\build.bat`

`build.bat` всегда устанавливает зависимости из `requirements-pinned.txt`. Если найден `vendor\wheels`, установка выполняется из локального wheel-кэша (`--no-index --find-links`) без загрузки из интернета.
