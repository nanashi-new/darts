...
Это программа для судьи и тренера по дартсу. Она помогает вести игроков и турниры, быстро загружать результаты из Excel, автоматически считать рейтинг по местам и очкам, а затем выгружать готовые таблицы и отчёты.

Программа работает на Windows и не требует интернета.

## CI-пайплайн
Автоматическая проверка запускается в GitHub Actions workflow:
- [`.github/workflows/ci.yml`](.github/workflows/ci.yml)

Пайплайн проверяет типизацию (`python -m mypy app`), общий набор тестов (`pytest -q -rs`) и отдельный smoke-прогон экспорта PNG/PDF/XLSX.

## Поддерживаемые версии Python
- **Целевая версия:** Python **3.11.x**.
- **Поддерживаемый диапазон:** `>=3.11, <3.12`.
- В CI и статической типизации используется Python 3.11 (`setup-python`, `mypy.ini`).

## Правило по зависимостям (dev vs release)
- Для **разработки (dev)** используйте `requirements.txt` (допускаются диапазоны версий, например `>=`).
- Для локальных проверок используйте `requirements-dev.txt`: он добавляет `pytest` и `mypy` поверх runtime-зависимостей.
- Для **release/offline-сборок** обязателен `requirements-pinned.txt` (только фиксированные версии `==`).

## Установка (Windows)
1. Скачайте архив с программой.
2. Распакуйте архив в удобную папку (например, на рабочий стол или диск D).
3. Откройте папку и запустите файл программы (обычно `DartsLiga.exe`).

Если Windows покажет предупреждение при первом запуске, нажмите «Подробнее» → «Выполнить в любом случае».

## Offline install policy
Источник wheel-пакетов фиксирован следующим образом:

- **Основной вариант (A, принят по умолчанию): GitHub Release artifacts.**
  Wheel-кэш (`vendor/wheels` + `manifest.json`) публикуется как артефакт релиза и не хранится в git-истории.
- **Резервный вариант (B): Git LFS в `vendor/wheels/`.**
  Используется только если невозможно применять Release artifacts (например, закрытый контур без релизного процесса).

### Пошаговая процедура обновления wheel-пакетов
1. Обновите/проверьте фиксацию зависимостей в `requirements-pinned.txt` (только `==`).
2. На машине с интернетом запустите:
   - `scripts\prepare_offline_wheels.bat`
3. Скрипт:
   - очищает старые `.whl` в `vendor\wheels`;
   - скачивает **только** пакеты из `requirements-pinned.txt`;
   - формирует `vendor\wheels\manifest.json` (package/version/filename/sha256).
4. Опубликуйте `vendor\wheels` и `manifest.json` в **GitHub Release artifacts** выбранного релиза.
5. На оффлайн-машине положите `vendor\wheels` рядом с проектом и запустите:
   - `scripts\build.bat`
6. `build.bat` перед установкой проверяет, что версии и SHA-256 хэши wheel-файлов соответствуют `requirements-pinned.txt` и `manifest.json`.

Если `vendor\wheels` отсутствует, `build.bat` выполняет обычную online-установку из `requirements-pinned.txt`.
...
- служебные файлы для профилей импорта.
...
### Настройки
- Служебные действия для обслуживания базы.
- Пересчёт рейтинга и слияние дублей игроков.
...
4. Выполните повторный импорт через профиль или мастер сопоставления.
5. Запустите пересчёт ещё раз (турнир или всё).
